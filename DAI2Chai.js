async function Launcher() {
    ...
    const adaiAddress = require('./config.js')['config']['ADAIAddress']
    ...
    const ContractAdai = new web3.eth.Contract(adaiABI, adaiAddress)
    ...
    //addressForChai.endsWith(typeof String) === true ? addressForChai = await web3.eth.ens.getAddress(addressForChai) : addressForChai
    ...
    async function checkChaiAllowance() {
        if (await ContractDAI.methods.allowance(myAddress, chaiAddress).call() === new BigNumber(0)) {
            ...
            rawTransaction.data = await ContractDAI.methods.approve(chaiAddress, bigAllowance).encodeABI()
            signedTx = await MyAccount.signTransaction(rawTransaction)
            sendTx = await web3.eth.sendSignedTransaction(signedTx.rawTransaction)
            .on('receipt', console.log)
            delete rawTransaction.nonce
    ...
    async function checkAdaiAllowance() {
        if (await ContractDAI.methods.allowance(myAddress, adaiAddress).call() === new BigNumber(0)) {
            // set max allowance for chai Contract
            bigAllowance = new BigNumber('115792089237316195423570985008687907853269984665640564039457584007913129639934')
            rawTransaction.data = await ContractDAI.methods.approve(adaiAddress, bigAllowance).encodeABI()
            signedTx = await MyAccount.signTransaction(rawTransaction)
            sendTx = await web3.eth.sendSignedTransaction(signedTx.rawTransaction)
            .on('receipt', console.log)
            delete rawTransaction.nonce
    ...
    async function DAItoAdai() {
        // converting DAI to Chai
        const DAIToSend = await ContractDAI.methods.balanceOf(myAddress).call()
        if (DAIToSend !== new BigNumber(0)) {
            delete rawTransaction.data
            rawTransaction.to = adaiAddress
            rawTransaction.gas = 500000
            rawTransaction.data = await ContractAdai.methods.deposit(adaiAddress, DAIToSend, 0);  //(myAddress, DAIToSend).encodeABI()
            ...
            nonce++
            rawTransaction.nonce = nonce
            console.log('DAI converted to ADAI, sending chai to storage address now...')
        }
    ...
    async function sendToStorage() {
        ...
        console.log("function sendToStorage  ", chaiBalance);
        
        if(chaiBalance === new BigNumber(0)) {
